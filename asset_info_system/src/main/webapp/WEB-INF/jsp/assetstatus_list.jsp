...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#delclick").click(function() {
			if ($("input[name='assetstatuschecked']:checked").size() == 0) {
				alert("选择不能为空!");
			} else {
				$("#del").fadeIn(200);
			} 
		...
		$("#delbutton").click(function() {
			...
			$("input[name='assetstatuschecked']:checked").each(function () {
				delIds.push(this.value);
            });
			$.ajax({
                url:"assetstatusbatchdelete",
                type:'post',
                data: {assetStatusIds:delIds.join(',')},
	    ...
	    $("#checkall").click(function(){ 
            if(this.checked){// 全选  
                $("input[name='assetstatuschecked']:checkbox").each(function(){  
                	this.checked=true;  
                });  
            } else {// 取消全选  
                $("input[name='assetstatuschecked']:checkbox").each(function(){  
...
<body>
	...
	<div class="rightinfo">
		...
		<table class="tablelist">
			<thead>
				<tr>
					...
					<th>资产状态<i class="sort"><img src="images/px.gif" /></i></th>
				</tr>
			</thead>
			<tbody>
				<c:forEach var="assetStatus" items="${assetStatus}" varStatus="status">
					<tr>
						<td><input name="assetstatuschecked" class="checkbox" type="checkbox" value="${assetStatus.id}" /></td>
		...
		<div class="edit" id="add">
			...
			<div class="formbody">
				<form action="assetstatusadd" method="post">
					资产状态：<input name="status" id="addstatus" class="select1"/>
					<div class="editbtn">
